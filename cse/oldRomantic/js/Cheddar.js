...
DisplayObject = (function() {
	...
	function DisplayObject() {
		...
		this.prerenderEventList  = new Array();
		...
		this.__defineGetter__ ("globalScaleX", function () {
			if (this.parent != null) {
				return this.scaleX * this.parent.globalScaleX
			} else {
				return this.scaleX
			}
		})
		
		this.__defineGetter__ ("globalScaleY", function () {
			if (this.parent != null) {
				return this.scaleY * this.parent.globalScaleY
			} else {
				return this.scaleY
			}
		})
		
		this.__defineGetter__ ("actualWidth", function () {
			return this.globalScaleX * this.width;
		})
		this.__defineGetter__ ("actualHeight", function () {
			return this.globalScaleY * this.height;
		})
		...
		this.hitTest = function (point) {
			...
			//var maxX = this.globalx + this.width / 2;
			//var minX = this.globalx - this.width / 2;
			/*
			for (var i=0; i < this.numChildren; i++) {
				var child = this.getChildAt(i);
				if (child.globalx + this.globalx + child.width / 2 > maxX) {
					maxX = child.globalx + this.globalx + child.width / 2
				}
				if (child.globalx + this.globalx - child.width / 2 < minX) {
					minX = child.globalx + this.globalx - child.width / 2
				}
			}
			*/
			
			//var maxY = this.globaly + this.height / 2;
			//var minY = this.globaly - this.height / 2;
			/*
			for (var i=0; i < this.numChildren; i++) {
				var child = this.getChildAt(i);
				if (child.globaly + this.globaly + child.height / 2 > maxY) {
					maxY = child.globaly + this.globaly + child.height / 2
				}
				if (child.globaly + this.globaly - child.height / 2 < minY) {
					minY = child.globaly + this.globaly - child.height / 2
				}
			}
			*/
			...
			//trace("A " + this.globalx, this.width, this.globaly, this.height)
			var w = this.actualWidth;
			var h = this.actualHeight;
			var centre = {x:(this.globalx +  w / 2 + this.globalx - w/2)/2,y:(this.globaly - h/2 + this.globaly + h / 2)/2}
			//trace(centre.x, centre.y)
			
			if (this.rotation != 0) {
				var theta = (-this.rotation) * Math.PI / 180;
				var d = {
					x:(point.x - centre.x) * Math.cos(theta) - Math.sin(theta) * (point.y - centre.y) + point.x,
					y:(point.x - centre.x) * Math.sin(theta) + Math.cos(theta) * (point.y - centre.y) + point.y
				}
			} else {
				d = {
					x:(point.x),
			...
			return (d.x > this.globalx - w / 2 && d.x < this.globalx + w / 2 && d.y > this.globaly - h / 2 && d.y < this.globaly + h / 2);
		}
	}
	
	DisplayObject.prototype.applyProperties = function (opts) {
		for ( var i in opts ) {
			this[i] = opts[i];
		}
	}
	...
	DisplayObject.prototype.draw = function (context) {
		...
		if (this.visible) {
			...
			this.dispatchEvents("prerender",context);
			for (var i=0; i < this.numChildren; i++) {
	...
	DisplayObject.prototype.parse = function (data, callContext) {
	
		var _this = this;
		function Node () {
			return {string:"", children:[], parent:null}
		}
		...
		//get headers
		headers = getHeaders(data)//data.replace(/\n+/g,"").split(/[<>]/).filter(function(e){return e})
		var currentNode = new Node();
		var root = currentNode;
		root.string = headers[0];
		//construct tree
		for (var i = 1 ; i < headers.length; i++) {
			 if (headers[i].replace(/\s+/g, '')[0] == "/") {
		...
		function createObj ( node ) {
			...
			var objType = "";
			var args = [];
			var header = node.string;
			var obj;
			
			//make obj with args
			var inName = false;
			for (i = 0 ; i < header.length; i++) {
				if (!inName && header[i].match(/\s/)) {
					continue;
				} else if (inName && header[i].match(/\s/)) {
					break;
				}
				objType += header[i];
				inName = true;
			}
			header = header.slice(i,header.length)
			//get args
			var inArgs=false;
			for (i = 0 ; i < header.length; i ++) {
				if (header[i] == "&") {
			...
			trace("Making new " + objType + " with args " + args)
			var a = [callContext]
			for (i = 0 ; i < args.length; i++) {
				a.push(args[i])
			}
			args = a;
			obj = new (Function.prototype.bind.apply(callContext[objType], args));
			//apply id
			for ( i =0  ; i < header.length; i++) {
				if (header[i] == "$") {
					while (header[i] != "=") {
					...
					i += 1;
					header = header.slice(i,header.length)
					var pstr = getProperty(header)
					callContext[pstr] = obj
					while (i < header.length && !header[i].match(/\s/)) {
						i += 1;
					}
					header = header.slice (i, header.length);
					break;
			...
			for (i = 0 ; i < header.length; i ++) {
				if (!header[i].match(/\s/)) {
					...
					var typeofprop = ""
					if (name == "@") {
						createEvent(obj, header.slice(i - 1,header.length))
						trace("creating event " + header.slice(i - 1,header.length).replace(/[\s\@]/,''))
						while (header[i] != "=") {
							i += 1
						}
						while (header[i].match(/\s/)) {
							i += 1;
						}
						while (i < header.length && !header[i].match(/\s/)) {
							i += 1;
						...
						i +=1;
						while (i < header.length && !header[i].match(/\s/) && header[i] != "=") {
							name += header[i]
							i += 1;
						}
						i += 1;
						...
						if (header[i] == "[") {
							val = createArray(header.slice(i-1,header.length))
							var num = 1;
							i += 1;
							while (num > 0) {
								if (header[i] == "[") {
							...
							obj[name] = val;
							trace("Setting " + name + " to " + val)
						} else if (header[i] == "{") {
							createBinding(obj, header.slice(nameStart,header.length))
							var bindName = "";
							while (header[i] != "}") {
								bindName += header[i];
								i += 1;
							...
							i += 1;
							bindName = bindName.replace(/\s/,'')
							bindName = bindName.replace('{', '')
							trace("Binding " + name + " to " + bindName)
						} else {
							while (header[i].match(/\s/) || header == "=") {
								i += 1;
							}
							val = getProperty(header.slice(i,header.length))
							obj[name] = val;
							trace("Setting " + name + " to " + val)
							var inquotes = false
							while (i < header.length && !header[i].match(/\s/) && (!inquotes || header[i] != '"')) {
								if (!inquotes && header[i] == '"') {
									inquotes = true;
								}
								if (inquotes && header[i] == '"') {
									inquotes = false;
			...
			for (i = 0 ; i < node.children.length; i ++) {
				obj.addChild(createObj(node.children[i]));
			}
			return obj;
		}
		...
		function getProperty (string) {
			...
			for (var i = 0 ; i < string.length ; i++) {
				el = ""
				if (string[i] == '"') {
					el = createString(string.slice(i, string.length));
					return el;
					break;
				}
				
				if (string[i].match(/[1234567890\-\.]/)) {
					el = "";
					while (i < string.length && string[i].match(/[1234567890\-\.]/)) {
						el += string[i]
						i += 1;
					}
					return parseFloat(el)
				}
				while (i < string.length && !string[i].match(/\s/) && string[i] != ",") {
					el += string[i];
					i+= 1;
				}
				var path = el.split(".").slice(0,el.split(".").length - 1).join(".")
				var last = el.split(".")[el.split(".").length - 1];
				var o = getObjFromPath(callContext, path);
				return o[last]
			}
		}
		...
		function createArray (string) {
			...
			for (var i = 0 ; i < string.length; i ++) {
				...
				if (string[i] == "[") {
					el = createArray(string.slice(i, string.length));
					arr.push(el);
					i += 1;
					var num = 1;
					while (num > 0) {
		...
		function createBinding (obj, string) {
			var prop = ""
			var valObj = ""
			var inBin = false
			var inName = true;
			...
			for (var i = 0 ; i < string.length; i++) {
				...
				if (inBin) {
					if (i == string.length - 1 || string[i] == "}") {
						inBin = false;
						if (i == string.length - 1 && !string.match(/\s/) && string[i] != "}") {
							binName += string[i]
						}
						var path = binName.split(".").slice(0, binName.split(".").length - 1).join(".");
						var last = binName.split(".")[binName.split(".").length - 1];
						var propPath = prop.split(".").slice(0, prop.split(".").length - 1).join(".")
						var plast = prop.split(".")[prop.split(".").length - 1];
						var o = getObjFromPath(callContext, path);
						if (propPath) {
							var p = getObjFromPath(obj, propPath);
							Cheddar.bind(o,last,p, plast);
						} else {
							Cheddar.bind(o,last,obj, prop);
						}
						return;
					}
					if (!string[i].match(/\s/) && string[i] != "{") {
		...
		function createEvent (obj, string) {
			var i = 0;
			var type = "";
			var callback = "";
			var inType = false;
			...
			for (i = 0; i < string.length; i++) {
				...
				if (inCallback) {
					...
					callback += string[i];
					if (string[i].match(/\s/) || i == string.length  - 1) {
						callback.replace(/\s/, '');
						inCallback = false;
						var func = getObjFromPath(this, callback);
						obj.on(type, func);
						return;
					}
				}
		...
		//parse each header top down
		this.addChild(createObj( root ))
	}
	
	DisplayObject.prototype.Serialize = function () {
		//TODO
		//turn object into markup
	}
...
Slider = (function(DisplayObject) {
	...
	function Slider(src) {
		...
		this.on ("mousemove", function (e) {
			if (!(this.background.mouseDown || this.ball.mouseDown || this.ball2.mouseDown)) {
			...
			if (this.oldMouse.y == -1) {
				this.oldMouse.y = e.pageY;
				return;
			}
			
			//this.setValue (this.value + (e.pageX - this.oldMouse.x)/2.7 / this.scaleX);
			var dx = Math.cos(this.rotation / 180 * Math.PI)*(e.pageX - this.oldMouse.x);
			var dy = Math.sin(this.rotation / 180 * Math.PI)*(e.pageY - this.oldMouse.y);
			this.setValue (this.value + (dx/(this.width / 100) / this.scaleX)*((this.maximum - this.minimum) / 100));
			this.setValue (this.value + (dy/(this.width / 100) / this.scaleX)*((this.maximum - this.minimum) / 100));
			this.oldMouse.x = e.pageX
			this.oldMouse.y = e.pageY
		});
		...
		//Cheddar.bind(this.background, "dragging", this.ball2, "visible");
		...
		this.on ("enterframe", function (e) {
			if (this.imgToDraw != undefined && this.imgToDraw.image != undefined) {
				if (this.animating) {
					...
					while (this.lastFrameTime > this.timeForFrame()) {
						this.lastFrameTime -= this.timeForFrame();
						this.nextFrame();
					}
				}
...
/* TODO
	Options for every cheddar class as first argument
	optimisations:
		prerendering -> render complex stuff to a canvas as cache then just drawImage(cacheCanvas)
		redraw regions -> only redraw changed areas
		use other canvases for layering
	...
	Stuff to add:
		Ability to define with JSON like mxml with binding
		target
...
/* !!!!ERRORS!!!!
	...
	mouse drag events are not working
