...
JSCovReporter = Backbone.View.extend({
    ...
    report: function () {
        ...
        var coverObjArr = [];
        var reportCache = {};
        _.each(this.coverObject, function(val, key) {
          coverObjArr.push({key : key, val : val});
          var file = new JSCovFileReporter({ coverObject: val });
          file.report();
          reportCache[key] = Math.round(file.pass / file.total * 100);
        }, this);
        
        coverObjArr.sort(_.bind(function(a, b) {
          var percA = reportCache[a.key];
          var percB = reportCache[b.key];
          return percA - percB;
        }, this));
        
        _.each(coverObjArr, function(coverObj, key) {
            var fileReporter = new JSCovFileReporter({ coverObject: coverObj.val });
            ...
            // Title
            result += '<h2 id="' + key + '" class="file-title">' + key + '</h2>';
            // Stats
            ...
            // Menu index
            index += '<li><span class="cov ' + type_coverage + '">' + percentage + '</span><a href="#' + key+ '">' + key + '</a></li>';
        }, this);
